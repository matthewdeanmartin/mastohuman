...
def serve_directory(directory: Path, host: str, port: int) -> socketserver.TCPServer:
    ...
    httpd = socketserver.TCPServer(
        (host, port), lambda *a, **kw: handler(*a, directory=str(directory), **kw)
    )
    return httpd
...
def main() -> int:
    ...
    p.add_argument(
        "--dir", default="docs", help="Directory to serve (static output folder)"
    )
    ...
    p.add_argument(
        ...
        action="store_true",
        default=True,
        help="Open browser (default)",
    )
    p.add_argument(
        "--no-open", dest="do_open", action="store_false", help="Do not open browser"
    )
    p.add_argument(
        "--delay", type=float, default=0.1, help="Seconds to wait before opening"
    )
    args = p.parse_args()
